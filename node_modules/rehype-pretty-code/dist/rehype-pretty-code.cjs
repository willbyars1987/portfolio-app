...
function splitElement({
  ...
  if (isElement(elementToWrap) && elementToWrap.children?.[0]?.type !== 'text' || ignoreWord) {
 ...
 */
function nextElementMaybeContinuesWord({
  elements,
  nextIndex,
  remainingPart
}) {
...
function getElementsToHighlight(element, word, startIndex = 0, ignoreWord = false) {
  const toWrap = [];
  let wordSoFar = '';
  if (element.children) {
    ...
    for (let i = startIndex; i < elements.length; i++) {
      ...
      if (word.startsWith(wordSoFar + content)) {
        ...
        if (nextElementMaybeContinuesWord({
          elements,
          nextIndex: i + 1,
      ...
      if (partialMatch) {
        ...
        if (nextPart !== '' && getContent(elements[i + 1]) && !nextElementMaybeContinuesWord({
          elements,
          nextIndex: i + 1,
...
function wrapHighlightedWords(parentElement, elementsToWrap, options, ignoreWord, onVisitHighlightedWord) {
  ...
  if (elementsToWrap.length > 1) {
    parentElement.children.splice(elementsToWrap[0].index, elementsToWrap.length, {
      ...
      tagName: 'span',
      properties: {
        'data-rehype-pretty-code-wrapper': true
      },
    ...
    onVisitHighlightedWord?.(parentElement.children[elementsToWrap[0].index], options.wordIdsMap.get(wordStr));
  } else {
    const [{
      element
    }] = elementsToWrap;
    ...
    onVisitHighlightedWord?.(element, options.wordIdsMap.get(textElement.value));
 ...
 */
function wordHighlighter(element, words, options, onVisitHighlightedWord) {
  const {
    wordNumbers = []
  } = options;
  ...
  words.forEach((word, index) => {
    if (word && textContent?.includes(word)) {
      ...
      while (textContent.includes(word)) {
        ...
        const ignoreWord = currentWordRange.length > 0 && !currentWordRange.includes(options.wordCounter.get(id) ?? -1);
        const elementsToWrap = getElementsToHighlight(element, word, startIndex, ignoreWord);

        // maybe throw / notify due to failure here
        if (elementsToWrap.length === 0) break;
        wrapHighlightedWords(element, elementsToWrap, options, ignoreWord, onVisitHighlightedWord);
        ...
        textContent = element.children?.map(childNode => {
          ...
          if (!hasOwnProperty(props, 'rehype-pretty-code-visited') && !hasOwnProperty(props, 'data-rehype-pretty-code-wrapper')) {
  ...
  return async tree => {
    ...
    visit(tree, 'element', (element, index, parent) => {
      ...
      element.tagName === 'pre' && Array.isArray(element.children) && element.children.length === 1 && isElement(element.children[0]) && element.children[0].tagName === 'code' && typeof element.children[0].properties === 'object' && Array.isArray(element.children[0].properties.className) && typeof element.children[0].properties.className[0] === 'string' && element.children[0].properties.className[0].startsWith('language-')) {
        ...
        const metastring = isElement(codeElement) ? codeElement.data?.meta ?? codeElement.properties?.metastring ?? '' : '';
        ...
        Object.entries(trees).forEach(([, tree]) => {
          ...
          visit(tree, 'element', element => {
            ...
            if (Array.isArray(element.properties?.className) && element.properties?.className?.[0] === 'line') {
              ...
              wordHighlighter(element, words, wordOptions, onVisitHighlightedWord);
